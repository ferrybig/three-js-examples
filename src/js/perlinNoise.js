...
class NoisePoint {
	...
	calculateForPoint(x, y) {
		...
		if (xDiff < -this.strongess || xDiff > this.strongess || yDiff < -this.strongess || yDiff > this.strongess || xDiff ** 2 + yDiff ** 2 > this.strongess ** 2) {
			return 0;
		}
		const distance = this.factor === 2 ? xDiff ** 2 + yDiff ** 2 : Math.sqrt(xDiff ** 2 + yDiff ** 2) ** this.factor;
...
export default class PerlinNoise {
	...
	noiseForRange(x, y) {
		...
		for (let rx = -20; rx < 20; rx++) {
			const nx = Math.sin(rx + x) * 17;
			for (let ry = -20; ry < 20; ry++) {
				const ny = Math.cos(ry + y) * 19;
				this.random.seed(Math.round(Math.sin(this.seed + nx + ny) * 1000));
				if (this.random.random() > 0.15) {
					noise.push(new NoisePoint(
							rx + x,
							ry + y,
							this.random.random() * 20,
							this.random.random() * 10,
	...
	start() {
		...
		loop.addTask('update', () => {
			...
			for (let i = tick * tilesPerTick - tilesPerTick; i < max; i++) {
				let color = Math.round(this.noise(pixels[i].x, pixels[i].y) + 128);
				if(color < 0)
					color = 0;
				if(color > 255)
					color = 255;
